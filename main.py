...
def run_slam(src_dir, log_dir, idx, split):
    ...
    slam = slam_t(resolution=0.05, Q=np.diag([2e-3, 2e-3, 1e-3]))
    slam.read_data(src_dir, idx, split)
    T = len(slam.lidar)
    ...
    for i in range(n):
        p[0, i] = random.uniform(-2, 2)
        p[1, i] = random.uniform(-2, 2)
        p[2, i] = random.uniform(-np.pi/8, np.pi/8)
    slam.init_particles(n, p, w)
    ...
    # plot the binary map (environment) and robot estimated path (position)
    environment, = plt.plot(np.array([0]), 'o', markersize=1, color='black')
    position, = plt.plot(np.array([0]), np.array([0]), '*', markersize=2, color='red')
    # free, = plt.plot(np.array([0]), np.array([0]), 'o', markersize=0.2, color='blue')     # plot each laser scan
    # all_free_environ, = plt.plot(np.array([0]), np.array([0]), '.', markersize=1, color='silver')

    step_size = 20  # 20 or 40 is pretty good step size
    ...
    for time in range(step_size, T, step_size):
        ...
        # x = slam.show_free[0, :]
        # y = slam.show_free[1, :]
        # free.set_data(y, 800 - x)

        # x = slam.all_free[0, :]
        # y = slam.all_free[1, :]
        # all_free_environ.set_data(y, 800-x)

        x, y = np.where(slam.map.cells == 1)
        environment.set_data(y, 800-x)

        x = slam.path[0, :]
        y = slam.path[1, :]
        position.set_data(y, 800-x)

        plt.draw()
        plt.pause(0.01)

    # figure will be saved to logs folder, be sure to change picture name before running this again
...
if __name__=='__main__':
    # modify the argument here to run it in different mode
    # main('./', 'logs', 0, 'train', 'dynamics')
    # main('./', 'logs', 0, 'train', 'observation')
    main('./', 'logs', 3, 'train', 'slam')      # choose 0, 1, 2, 3 for which map to run full slam on
